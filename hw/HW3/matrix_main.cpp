...
void StudentTest(){
	//creates a 2x2 matrix of 0's. set m2 equal to m1 and test if they are the same. 
	//print both matrixes.
	Matrix m1(2, 2, 0);
	std::cout << m1 << std::endl;
	Matrix m2;
...
//Write this if you write resisze()
void ExtraCreditTest(){
	Matrix m20(2, 2, 0);
	m20.resize(5, 5, 3);
	assert(m20.get_num(0,0) == 0);
	assert(m20.get_num(3,3) == 3);
	std::cout << m20 << std::endl;
	m20.resize(3, 3, 4);
	std::cout << m20 << std::endl;
	m20.resize(0, 0 ,0);
	std::cout << m20 << std::endl;
