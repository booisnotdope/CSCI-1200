...
public:
  // CONSTRUCTOR & DESTRUCTOR
  pBalloon(const std::string& name_) : name(name_) {
    std::cout << "Balloon constructor " << name << std::endl;
    num_ropes = 0;
  ...
  // add a rope connecting this balloon to another
  void addRope(dsSharedPtr<pBalloon>& b) { 
    assert (num_ropes < MAX_NUM_ROPES);
    ropes[num_ropes] = b;
    num_ropes++;
  }
  // detach a rope connecting this balloon to another
  void removeRope(dsSharedPtr<pBalloon>& b) { 
    for (int i = 0; i < MAX_NUM_ROPES; i++) {
      if (ropes[i] == b) { ropes[i] = ropes[num_ropes-1]; num_ropes--; break; }
    }
  }
...
private:
  ...
  int num_ropes;
  // a dynamically allocated C-style array of ropes connecting to other Balloons
  dsSharedPtr<pBalloon> ropes[MAX_NUM_ROPES]; 
};
...
*/
  ...
  //
    ...
    dsSharedPtr<pBalloon> georgette(new pBalloon("Mr Potato Head"));
    dsSharedPtr<pBalloon> henry(new pBalloon("Snoopy"));
    georgette->addRope(henry);
    georgette->print();
    henry->print();

    dsSharedPtr<pBalloon> isabelle(new pBalloon("Shrek"));
    henry->addRope(isabelle);
    isabelle = dsSharedPtr<pBalloon>(new pBalloon("Barney the Purple Dinosaur"));
    georgette->addRope(isabelle);
