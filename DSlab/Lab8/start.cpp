...
void print_grid_2(const std::vector<std::vector<GRID_STATUS> > & blocked_grid,
                unsigned int start_x, unsigned int start_y, std::vector<Point> &valid_path) {
  ...
  for (unsigned int y=0; y<blocked_grid[0].size(); ++y) {
    for (unsigned int x=0; x<blocked_grid.size(); ++x) {
      for (unsigned int i = 0; i < valid_path.size(); i++){
        if (valid_path[i].x == x && valid_path[i].y == y){
          std::cout << " $";
          something = true;
          break;
        }
...
int find_path(int x, int y, std::vector<std::vector<GRID_STATUS> > blocked_grid, std::vector<Point> &valid_path){
    if ( x < 0 || y < 0 || blocked_grid[x][y] == GRID_BLOCKED){
        // valid_path.clear();
        return 0;
    } else if (x == 0 && y == 0){
      valid_path.push_back(Point(x, y));
      return 1;
    } else {
      valid_path.push_back(Point(x, y));
    }
    return find_path(x, y - 1, blocked_grid, valid_path) + find_path(x - 1, y, blocked_grid, valid_path);
}
...
int main(int argc, char* argv[]) {
  ...
  std::vector<Point> valid_paths;
  std::cout << std::endl;
  int paths = find_path(start_x, start_y, blocked_grid, valid_paths);
  std::cout << paths << std::endl;
  
  // for (unsigned int i = 0; i < valid_paths.size(); i++){
  //   std::cout << '(' << valid_paths[i].x << ',' << valid_paths[i].y << ") ";
  // }
  std::vector<Point> valid_path;
  for (unsigned int i = 0; i < valid_paths.size(); i++){
    if (valid_paths[i].x == 0 && valid_paths[i].y == 0){
      for (int j = 0; j < start_x + start_y; j++){
        std::cout << '(' << valid_paths[i - j].x << ',' << valid_paths[i - j].y << ") ";
        valid_path.push_back(valid_paths[i - j]);
      }
      std::cout << std::endl;
      break;
    }
  }
  print_grid_2(blocked_grid, start_x, start_y, valid_path);
